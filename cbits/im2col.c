...
void im2col_cpu(const RealNum* data_im, const int channels,
  ...
  const int fitting_height_i_max = (height - kernel_h) / stride_h;
  const int fitting_width_i_max = (width - kernel_w) / stride_w;
  
  const int block_size = (fitting_width_i_max + 1) * channels * kernel_h * kernel_w;

  // TODO: Disable parallel for small matrices
#pragma omp parallel for
  for (int fitting_height_i = 0; fitting_height_i <= fitting_height_i_max; fitting_height_i++) {
    int fitting_height = fitting_height_i * stride_h;
    RealNum* block_data_col = data_col + fitting_height_i * block_size;
    for (int fitting_width_i = 0; fitting_width_i <= fitting_width_i_max; fitting_width_i++) {
      int fitting_width = fitting_width_i * stride_w;
      for (int channel = 0; channel < channels; channel++) {
        for (int kernel_row = 0; kernel_row < kernel_h; kernel_row++) {
          int input_row = fitting_height + kernel_row;
          const RealNum* cpy_start = &data_im[fitting_width + input_row * width + channel_size * channel];
          memcpy(block_data_col, cpy_start, sizeof(RealNum) * kernel_w);
          block_data_col += kernel_w;
        }
      }
    }
  }
