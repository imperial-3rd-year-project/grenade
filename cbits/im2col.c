  ...
  for (int fitting_height = 0; fitting_height <= (height - kernel_h); fitting_height += stride_h) {
    for (int fitting_width = 0; fitting_width <= (width - kernel_w); fitting_width += stride_w) {
      for (int channel = 0; channel < channels; channel++) {
        for (int kernel_row = 0; kernel_row < kernel_h; kernel_row++) {
          int input_row = fitting_height + kernel_row;
          memcpy(data_col, data_im + input_row * width + channel_size * channel, sizeof(RealNum) * kernel_w);
          data_col += kernel_w;
        }
      }
    }
  ...
  for (int fitting_height_i = 0; fitting_height_i <= fitting_height_i_max; fitting_height_i++) {
    ...
    for (int fitting_width_i = 0; fitting_width_i <= fitting_width_i_max; fitting_width_i++) {
      for (int channel = 0; channel < channels; channel++) {
        for (int kernel_row = 0; kernel_row < kernel_h; kernel_row++) {
          int input_row = fitting_height + kernel_row;
          memcpy(block_data_col, data_im + input_row * width + channel_size * channel, sizeof(RealNum) * kernel_w);
